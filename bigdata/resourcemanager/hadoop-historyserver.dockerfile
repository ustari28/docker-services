#docker build -f hadoop-historyserver.dockerfile --rm -t bigdata/hadoop-historyserver:1.0 .
...
LABEL version=1.0
LABEL description="Hadoop History Server"
LABEL bigdata=historyserver
...
COPY start-historyserver.sh $HOME_HOST/start-historyserver.sh
...
RUN chmod 755 $HOME_HOST/start-historyserver.sh && \
...
ENV CVS_RSH=ssh
ENTRYPOINT ["/user/hadoop/start-historyserver.sh"]
#docker network create --driver bridge hadoop-cluster
#docker run -dit -p 19888:19888 --name historyserver --network=hadoop-cluster --link namenode --link resourcemanager -h historyserver bigdata/hadoop-historyserver:1.0 "/bin/bash"
