#docker build -f hadoop-nodemanager.dockerfile --rm -t bigdata/hadoop-nodemanager:1.0 .
#docker build -f hadoop-nodemanager.dockerfile --rm -t bigdata/hadoop-nodemanager:lastest .
FROM ubuntu
LABEL maintainer="ustargab@gmail.com"
LABEL version=1.0
LABEL description="Hadoop NodeManager"
LABEL bigdata=nodemanager
...
COPY start-nodemanager.sh $HOME_HOST/start-nodemanager.sh
...
RUN chmod 755 $HOME_HOST/start-nodemanager.sh && \
...
ENV CVS_RSH=ssh
ENTRYPOINT ["/user/hadoop/start-nodemanager.sh"]
#docker network create --driver bridge hadoop-cluster
#docker run -dit --name nodemanager1 --network=hadoop-cluster --link resourcemanager -h nodemanager1 bigdata/hadoop-nodemanager:1.0 "/bin/bash"
